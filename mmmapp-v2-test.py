...
def page_forecast():
    ...
    sns.barplot(x=weekly_data["Week"].iloc[-12:], y=weekly_data["Predicted_Revenue"].iloc[-12:], ax=ax2)
    ax2.set_ylim(0, 100)
    ax2.set_title("Predicted Revenue Trend")
    st.pyplot(fig2)
    
    roi_per_channel = {channel: (feature_importance_dict.get(channel, 0) * weekly_data["Predicted_Revenue"].sum()) / (weekly_data[channel].sum()) for channel in spend_factors}
